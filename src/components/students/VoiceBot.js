  ...
  const handleSpeechInput = async (transcript) => {
    ...
    // e.g. "read skits", "read stock sentences", "read stories" → reads current page OR first item of that category in current class
    {
      ...
      if (spokenCategory && (isReadCommand || isPracticeCommand)) {
        ...
        speak(`Please go to a class first. Say: Go to Class 8, then say read ${spokenCategory === "stockSentences" ? "stock sentences" : spokenCategory === "actionWords" ? "action words" : spokenCategory}.`, () => {
          setAppState("LISTENING");
        });
        return;
      }
